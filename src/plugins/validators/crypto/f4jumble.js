...
const G = (i, m, ellR) => {
  const aux = reduce(
    ...
    new Uint8Array([]),
    range(0, Math.floor((ellR + ELL_H - 1) / ELL_H))
  )

  return slice(0, ellR, aux)
}
...
const f4Jumble = input => {
  const ellM = size(input)
  const ellL = min([ELL_H, Math.floor(ellM / 2)])
  const ellR = ellM - ellL

  const a = new Uint8Array(slice(0, ellL, input))
  const b = new Uint8Array(slice(ellL, ellM, input))
  ...
  return new Uint8Array(concat(c, d))
}
...
const f4Unjumble = input => {
  ...
  const c = new Uint8Array(slice(0, ellL, input))
  const d = new Uint8Array(slice(ellL, ellM, input))
  ...
  return new Uint8Array(concat(a, b))
}
