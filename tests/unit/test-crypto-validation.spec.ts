...
test('Should validate BTC P2PKH', () => {
  ...
  const validatedMain = base58Validator('main', mainNetaddr, BTC.base58Opts)
  const validatedTest = base58Validator('test', testNetaddr, BTC.base58Opts)
...
test('Should validate LTC P2PKH', () => {
  ...
  const validatedMain = base58Validator('main', mainNetaddr, LTC.base58Opts)
  const validatedTest = base58Validator('test', testNetaddr, LTC.base58Opts)
  expect(validatedMain).toBe(true)
  expect(validatedTest).toBe(true)
})
...
test('Should validate DASH P2PKH', () => {
  ...
  const validatedMain = base58Validator('main', mainNetaddr, DASH.base58Opts)
  const validatedTest = base58Validator('test', testNetaddr, DASH.base58Opts)
  expect(validatedMain).toBe(true)
  expect(validatedTest).toBe(true)
})
...
test('Should validate ZEC P2PKH', () => {
  ...
  const validatedMain = base58Validator('main', mainNetaddr, ZEC.base58Opts)
  const validatedTest = base58Validator('test', testNetaddr, ZEC.base58Opts)
  expect(validatedMain).toBe(true)
  expect(validatedTest).toBe(true)
})
...
test('Should validate BTC bech32', () => {
  ...
  const validatedMain = bech32Validator('main', mainNetaddr, BTC.bech32Opts)
  const validatedTest = bech32Validator('test', testNetaddr, BTC.bech32Opts)
  expect(validatedMain).toBe(true)
  expect(validatedTest).toBe(true)
})
...
test('Should validate BTC bech32m', () => {
  ...
  const validatedMain = bech32mValidator('main', mainNetaddr, BTC.bech32Opts)
  const validatedTest = bech32mValidator('test', testNetaddr, BTC.bech32Opts)
  expect(validatedMain).toBe(true)
  expect(validatedTest).toBe(true)
})
...
test('Should validate LTC bech32', () => {
  ...
  const validatedMain = bech32Validator('main', mainNetaddr, LTC.bech32Opts)
  const validatedTest = bech32Validator('test', testNetaddr, LTC.bech32Opts)
  expect(validatedMain).toBe(true)
  expect(validatedTest).toBe(true)
})
...
test('Should validate ZEC bech32', () => {
  ...
  const validatedMain = zecBech32Validator('main', mainNetaddr, ZEC.bech32Opts)
  const validatedTest = zecBech32Validator('test', testNetaddr, ZEC.bech32Opts)
  expect(validatedMain).toBe(true)
  expect(validatedTest).toBe(true)
})
...
test('Should validate XMR address', () => {
  ...
  const validatedMain = xmrValidator('main', mainNetaddr, XMR.opts)
  const validatedTest = xmrValidator('test', testNetaddr, XMR.opts)
  expect(validatedMain).toBe(true)
  expect(validatedTest).toBe(true)
})
...
test('Should validate ZEC bech32m', () => {
  ...
  const validatedMain = zecBech32mValidator('main', mainNetaddr, ZEC.bech32mOpts)
  const validatedTest = zecBech32mValidator('test', testNetaddr, ZEC.bech32mOpts)
  expect(validatedMain).toBe(true)
  expect(validatedTest).toBe(true)
})
...
test('Should fail length validation', () => {
  ...
  const validated = base58Validator('main', addr, opts)
  expect(validated).toBe(false)
})

test('Should throw invalid address checksum exception', () => {
  const addr = '3J98t1WpEZ73CNmQviecrnyiWrnqRhWN'
  const validated = base58Validator('main', addr, BTC.base58Opts)
  expect(validated).toBe(false)
})
